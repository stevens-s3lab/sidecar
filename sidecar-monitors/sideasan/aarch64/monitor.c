...
mecpy_reg_pair(char *addr, int lo_offset, int hi_offset)
{
	uint64_t vallo, valhi, val;

	memcpy(&vallo, (addr + lo_offset), 4);
	memcpy(&valhi, (addr + hi_offset), 4);
	val = (valhi << 32) | vallo;
	return val;
}
...
decoder_init(void)
{
	...
	/* set payload parser as default */
	asdec.parser_index = OPCODE_PARSER;

	/* set asan pkt index to the first word */
	asdec.pkt_index = 0;
}
...
void
asan_opcode_parser(ocsd_stm_pkt *pkt)
{
	/* reset pkt index */
	asdec.pkt_index = 0;
	...
	asdec.parser_index = pkt->payload.D32 & 0xff;
}
		...
		const void *p_packet_in)
{
	...
	{
		case OCSD_OP_DATA:
			/* we only need D32 pkts */
			if (pkt->type != STM_PKT_D32)
				break;
