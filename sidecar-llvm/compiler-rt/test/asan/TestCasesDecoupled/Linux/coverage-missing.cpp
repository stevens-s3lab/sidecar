...
// RUN: mkdir -p %t-dir
// RUN: cd %t-dir
// RUN: SIDECAR_BASE/sidecar/sidecar-monitors/sideasan/x86-64/monitor & %env_asan_opts=coverage=1:coverage_dir=%t-dir %run taskset -c 0 %t
// RUN: %sancov print *.sancov > main.txt
// RUN: rm *.sancov
// RUN: count 1 < main.txt
// RUN: SIDECAR_BASE/sidecar/sidecar-monitors/sideasan/x86-64/monitor & %env_asan_opts=coverage=1:coverage_dir=%t-dir %run taskset -c 0 %t x
// RUN: %sancov print *.sancov > foo.txt
// RUN: rm *.sancov
// RUN: count 3 < foo.txt
// RUN: SIDECAR_BASE/sidecar/sidecar-monitors/sideasan/x86-64/monitor & %env_asan_opts=coverage=1:coverage_dir=%t-dir %run taskset -c 0 %t x x
